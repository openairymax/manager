...
`manager/agent/` 包含 AgentRT 的 Agent 注册表配置，定义了 14 个 Agent（12 个已实现 + 2 个已规划）的完整注册信息。每个 Agent 包含角色定义、能力声明、双系统模型配置、权限要求、成本概览、信任指标和资源限制等维度，遵循 `schema/agent-registry.schema.json` 规范。
